...
const staticRegainGroup = document.getElementById("staticRegainGroup");

const cfmInput = document.getElementById("cfmInput");
const velocityInput = document.getElementById("velocityInput");
const frictionInput = document.getElementById("frictionInput");
const enteringVelocityInput =
    document.getElementById("enteringVelocityInput");
const staticRegainInput =
    document.getElementById("staticRegainInput");
const regainEfficiencyInput =
    document.getElementById("regainEfficiencyInput");
const results = document.getElementById("results");

function updateAirflowMethodGroups() {
    const method = methodSelect.value;

    velocityGroup.style.display =
        method === "constantVelocity" ? "block" : "none";

    frictionGroup.style.display =
        method === "equalFriction" ? "block" : "none";

    staticRegainGroup.style.display =
        method === "staticRegain" ? "block" : "none";
}

function showAirflowError(message) {
    results.innerHTML = `
...
function updateCalculator() {
    ...
    if (!Number.isFinite(cfm) || cfm <= 0) {
        showAirflowError("Enter a valid airflow greater than zero.");
        return;
    }

    if (methodSelect.value === "constantVelocity") {
        const data = calculateConstantVelocity(
            cfm,
            Number(velocityInput.value)
        );

        if (!data) {
            showAirflowError("Enter a valid target velocity greater than zero.");
            return;
        }
    ...
    if (methodSelect.value === "equalFriction") {
        const data = calculateEqualFriction(
            cfm,
            Number(frictionInput.value)
        );

        if (!data) {
            showAirflowError("Enter a valid friction rate greater than zero.");
            return;
        }

        results.innerHTML = buildEqualFrictionResults(data);
        return;
    }

    const data = calculateStaticRegain(
        cfm,
        Number(enteringVelocityInput.value),
        Number(staticRegainInput.value),
        Number(regainEfficiencyInput.value)
    );

    if (!data) {
        showAirflowError(
            "Enter valid static-regain values. Efficiency must be between 1% and 100%."
        );
        return;
    }

    if (data.invalidReason) {
        showAirflowError(data.invalidReason);
        return;
    }

    results.innerHTML = buildStaticRegainResults(data);
}
...
[
    ...
    velocityInput,
    frictionInput,
    enteringVelocityInput,
    staticRegainInput,
    regainEfficiencyInput
].forEach(element => {
    element.addEventListener("input", updateCalculator);
});

methodSelect.addEventListener("change", () => {
    updateAirflowMethodGroups();
    updateCalculator();
});
...
function findAutoClrForSolvedAngle(
    offset,
    overallLength,
    bendDimension
) {
...
function findAutoClrForKnownAngle(
    offset,
    elbowAngle,
    bendDimension
) {
    ...
    for (const multiplier of AUTO_CLR_MULTIPLIERS) {
        ...
        if (straight >= -0.001) {
            return { radius, angle: elbowAngle, multiplier };
        }
    }
...
calculateOffsetBtn.addEventListener("click", () => {
    ...
    if (offsetMode.value === "solveAngle") {
        ...
        if (clrMultiplier.value === "auto") {
            ...
            /*
             * Shop Auto CLR sequence. Prefer 1.00× and step down only
             * when the requested geometry will not fit. Never select an
             * in-between value such as 0.62×.
             */
            const autoSelection =
            ...
            centerlineRadius = autoSelection.radius;
            elbowAngle = autoSelection.angle;
